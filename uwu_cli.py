...
from utils.diff_engine import show_preview
...
class UwUCLI:
    def __init__(self):
        ...
        self.fx_manager = FXManager(mode=self.config.get('fx_mode', 'soft'))
    ...
    def _execute_telegram_command(self, command: str) -> str:
        ...
        try:
            ...
            if command.lower().startswith("cursor:cmd "):
                ...
                import psutil
                return f"📊 CPU: {psutil.cpu_percent()}% | Mem: {psutil.virtual_memory().percent}%"
            elif command.lower() == "/logs":
                return "📚 (Recent activity tail mock)"
